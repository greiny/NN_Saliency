...
bool neuralNetwork::saveWeights(double hTAcc, double hGAcc, long hEpoch)
{
	...
	if ( outputFile.is_open() && loggingEnabled )
	{
		...
		if (nLayer==0) {
			for ( int i=0; i < nInput; i++ )
				for ( int j=0; j < nOutput; j++ ) outputFile << wInputOutput[i][j] << ","; // nInput*nHidden
			outputFile << endl;
		}
		else {
			//output weights
...
#if 1
		outputNeurons = sotfMax(outputNeurons);
		//check all outputs against desired output values
		for ( int k = 0; k < nOutput; k++ )
			if ( outputNeurons[k] != set[tp]->target[k] ) correctResult = false;
...
void neuralNetwork::initializeWeights()
{
	...
	if (nLayer==0) {
		for(int i=0; i<nInput; i++){
			for(int j=0; j<nOutput; j++)
				wInputOutput[i][j] = (((double)(rand()%100)+1)/100*2*rH)-rH;
		}
	}
	else {
		for(int i=0; i<=nInput; i++)
